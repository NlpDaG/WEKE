...
def sum_cite_edge_freq(file_name, data_dir, cite_type, window=2):
    ...
    for cite_name in cite_list:
        cite_text = filter_text(
            read_file(os.path.join(cite_dir, cite_name)), with_tag=False)
        cite_edge_freq = single_cite_edge_freq(
            target, cite_text, window=window)
        for key in cite_edge_freq:
            cite_edge_freqs[key] = cite_edge_freqs.get(
                key, 0) + cite_edge_freq[key]
...
def main(dataset, window):
    ...
    for file_name in file_names:
        ...
        cited_edge_freq = sum_cite_edge_freq(
            file_name, data_dir, 'cited', window=window)
        citing_edge_freq = sum_cite_edge_freq(
            file_name, data_dir, 'citing', window=window)
        save_edge_features(file_name, data_dir, edge_freq,
                           cited_edge_freq, citing_edge_freq)
