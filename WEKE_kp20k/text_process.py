...
def get_phrases(pr, graph, doc_path, ng=2, pl2=0.6, pl3=0.3, with_tag=True):
    ...
    for n in range(2, ng + 1):
        for ngram in ngrams(tokens, n):
            ...
            if all(is_word(token) for token in ngram):
                head, tail = normalized_token(
                    ngram[0]), normalized_token(ngram[-1])

                if head in edges and tail in edges[head] and pos_tag([ngram[-1]])[0][1] != 'JJ':
                    phrase = ' '.join(list(normalized_token(word)
                                           for word in ngram))
                    #phrase = ' '.join(ngram)
                    phrases.add(phrase)
    ...
    # 若不取词干，此处需要对pr中的key做词形还原处理。
    sorted_word = sorted(pr.items(), key=lambda d: d[1], reverse=True)
    out_sorted = sorted(sorted_phrases + sorted_word,
                        key=lambda d: (d[1], d[0]), reverse=True)
    return out_sorted
    # 每次得到的keyphrases的排序不固定，因为不同词组可能相同得分(单复数问题，同词形问题)
